...
class BertNerModel(BaseModel):
    ...
    def forward(self,
        ...
        seq_out = bert_outputs[0]  # [batch-size, max_len, 768]
        ...
        if self.args.use_lstm == 'True':
            ...
            bio_seq_out = bio_seq_out.contiguous().view(
                batch_size,
                self.args.max_seq_len,
                -1
            )
            att_seq_out = att_seq_out.contiguous().view(
                batch_size,
                self.args.max_seq_len,
                -1
            )
            # seq_out = seq_out.contiguous().view(batch_size, self.args.max_seq_len, -1)
            # #[batch-size, max_len, num_tags]
        else:
            seq_out = self.mid_linear(seq_out)  # [batch-size, max_len, 256]
            bio_seq_out = self.bio_linear(seq_out)
            att_seq_out = self.att_linear(seq_out)
        ...
        if self.args.use_crf == 'True':
            ...
            active = torch.argmax(bio_seq_out, -1).view(-1) > 0
...
if __name__ == '__main__':
    ...
    args.bio_tags = args.bio_tags 
    args.att_tags = args.att_tags
    args.use_lstm = 'True'
    args.use_crf = 'True'
